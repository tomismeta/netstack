...
| NET Advance terms, positions, holders, capacity and parameters | [Products](references/products.md), `advance` in the [runner guidance](references/integrations.md#live-analytics-execution-limits); offline `advance provenance --json`, optional live `--detail summary`; `analytics.advance` in [Address index](assets/address-index.json) | Explorer-bytecode-backed settlement is not pinned live state or dependency verification. `paid` mixes cash and collateral credit; Treasury entitlement is not remittance, and pledged wsNET is not protocol backing. Preserve fresh event/count reconciliation and the unofficial-analysis disclaimer. |
