...
Export and archive read **committed blob bytes**, never dirty worktree bytes, and require that committed manifest/content to verify. Their destination must not exist and its parent must already be a real directory; symlink ancestors, traversal and overwrite are refused. Export automatically creates a sibling **`netstack.receipt.json`** for an output directory named `netstack`; both names must be unused, including no dangling symlinks. Its JSON result exposes the absolute `receipt` path. The sidecar reuses the export report schema: full reviewed `commit`, `version`, `manifest_sha256`, `runtime_sha256`, output/receipt paths and verification details. Compare its version and two hashes with the installed verifier's output, and its full SHA with your independently reviewed commit.

The receipt stays **beside, never inside**, the strict runtime tree; adding it inside would make installed verification fail. Export publishes the complete receipt atomically without replacing an existing name, writes the discovery marker only after the other files and receipt, and removes its newly created tree/receipt/staging on a handled failure. This is not a two-path crash-atomic transaction: after interruption or power loss, inspect both paths and verify before treating the install as complete. The receipt records checksum-bound provenance, **not a signature, authentication, proof of independent review or host-loaded identity**. Preserve it as separate installation evidence; export does not modify deterministic runtime manifest bytes to embed a containing commit.

Archives contain one `netstack/` directory with sorted members, uncompressed `ZIP_STORED` bytes and fixed timestamps/permissions, yielding identical bytes across supported hosts for the same committed package. Archives do not include the host-specific installation receipt. Keep both root `LICENSE` and `assets/LICENSE.txt`.
