...
Export and archive read **committed blob bytes**, never dirty worktree bytes, and require that committed manifest/content to verify. Their destination must not exist and its parent must already be a real directory; symlink ancestors, traversal and overwrite are refused. Export automatically creates a sibling **`netstack.receipt.json`** for an output directory named `netstack`; both names must be unused, including no dangling symlinks. Its CLI JSON result reports the actual absolute `output` and `receipt` paths. The portable sidecar contains only the existing identity fields: full reviewed `commit`, `version`, `manifest_sha256`, `runtime_sha256`, and a concise integrity-not-authenticity `note`; it stores no host paths. Compare its version and two hashes with the installed verifier's output, and its full SHA with your independently reviewed commit.
