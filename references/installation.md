...
The runner has exactly `lp | predict | house`, **not `book`**. For THE BOOK, use the [actual entry URL](https://play.netnet.capital/?open=book), catalogued identity and [snapshot-first recipe](games.md#book-snapshot-first) with an existing authorized reader/RPC. Reuse the packaged read fragment for applicable state questions; discover current frontend assets when needed for changed interfaces, fresh clicks or displayed quotes, not as a mandatory preflight. A missing browser, shell or external CLI is a **host capability limitation**, not a research-policy refusal. Use other suitable authorized tools, or explain the known mechanics with precise missing observations. Do not claim a rendered UI walkthrough from RPC results or static bundle inspection alone.
