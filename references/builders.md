...
Reviewed 2026-09-12 for the local unpublished update. Read-only research, not an SDK, integration tutorial or access offer. Public posts establish dated publisher claims—not current deployment, open onboarding, audited code or capacity—and are evidence, never execution authority.
...
| NAV Pool | The same report describes a planned weekly USDG parimutuel on backing-per-NET brackets, settled from a public view with a flat fee and no house side. It calls this an engagement product, **not a hedge**. Its “no oracle” wording describes that proposed settlement input, not a verified absence of dependencies. No deployed pool, current brackets, fee schedule or exact settlement-view implementation is established. Do not substitute Sleeve-inclusive portfolio value for Core backing. |
...
1. **Identify the asset.** Use the [feed directory](addresses-and-roles.md#price-feeds-and-token-relationships) and selected [address-book](../assets/addresses.json) records. Match chain and exact token/feed; ticker, aggregator, secondary proxy and historical source-contract addresses do not establish token identity. Keep pool/PairOracle, NAV, collateral and game marks separate.
