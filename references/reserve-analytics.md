...
Recognition covers the reviewed executable calculation and its dependencies, registry and selection rules—not a component's name or a visible panel. Only narrowly reviewed presentational differences may be normalized. Changed unrecognized accounting, dependencies or minification, unavailable source, or unsupported page structure cannot inherit the old review. Authorized agents may investigate new source statically, but must establish its calculation before reporting a current publisher total; this helper is not a general JavaScript interpreter.

The [reviewed calculation](https://app.netnet.capital/assets/index-CmCKnROW.js) applies separate selection and valuation rules to wallet holdings, Credit, LP positions, product pots and own-token exposure, then adds Core RFV. Collection and publisher selection are different scopes: a registry entry is neither proof of ownership nor an exhaustive holdings list. The [supported methodology record](../assets/analytics/reports-methodology.json) records those rules, including principal-versus-fee distinctions and selected position identities; live ownership, quantities and marks still require RPC.
