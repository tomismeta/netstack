...
Use for LP earnings, uncollected fees, returned principal and their possible contribution to Sleeve buybacks. This is a research recipe, not a position operator or transaction SDK. Public contract state/events are primary for contract-derived metrics; official sources support protocol claims. Dashboards are optional evidence where appropriate or when explicitly requested, never default inputs or privileged authorities. Directory order does not drive retrieval, ranking or fallback. Follow the [research guardrail](../netstack-guardrail.md) and [Safety](safety.md): explain user-operated collection/liquidity mechanics concretely, but do not operate wallets, sign, submit transactions, mutate live-chain state or prepare execution-ready transaction artifacts. Separately validated read-only quotes and isolated nonbroadcast simulations are allowed; simulated outputs are not actual collectible value or cash receipts.

**Scope boundary:** this recipe is for **Uniswap v3 NFT positions**, principally the Manager Sleeve's RWA liquidity. [Retail NET/USDG liquidity](products.md#retail-netusdg-v2-liquidity-and-the-lp-zap) instead uses fungible **v2 LP receipts**, with pool fees accruing in reserves; the deployed-but-app-gated LP Zap is a separate entry design, not an NFPM position. [Predict's House Vault](products.md#netnet-predict-weekly-outcomes-and-house-vault) is outcome underwriting with queued entry/exit and losses, not AMM liquidity. Neither product uses the v3 tick/fee-growth reconstruction below. Do not interpret a v2 share valuation or House Vault fee display as v3 uncollected fees or a Core buyback budget.

Use [v2 liquidity analytics](liquidity-analytics.md) for NET/USDG provider holdings and pool-fee attribution, and [Predict/House analytics](predict-analytics.md) for wagers, queues and underwriting results. Those workflows have separate read/event interfaces and accounting units.

## 1. Fix scope and identify positions

Read `lp_inspection` in [address-index.json](../assets/address-index.json). Each `{file,id}` points to an existing canonical Sleeve, Uniswap V3 NonfungiblePositionManager (NFPM), factory or pool record. No address is duplicated here. Known pools are examples, not the Sleeve's complete holdings. Read [conventions](../assets/address-conventions.json) for scope and exact Robinhood Etherscan templates.
...
- Rediscover the full owned NFT set for **each new observation block**. Never reuse token IDs or a position count from an example or earlier report as the current universe. Rebalancing can burn/mint NFTs, transfer them, or change liquidity on an existing ID; re-read the position state even when an ID persists. Resolve each discovered position's pool through its token pair, fee and verified factory, including pools absent from `known_pools`. That list is a navigation hint, not a filter or allowlist. Report newly observed identities and unresolved asset classifications without inventing symbol/feed mappings or automatically editing the catalog.
...
Use host-authorized research tools and exact integers. The [official public endpoint](https://docs.robinhood.com/chain/connecting/#public-endpoints) is `https://rpc.mainnet.chain.robinhood.com`; it is rate-limited, not a production/archive guarantee. Archive-capable providers may be needed. Non-wallet authentication or local research resources may be used under existing host authorization and confidentiality controls; do not expose credentials or obtain broader access merely because a source requests it.

Set an explicit call/page budget appropriate to the request. Quick-pass defaults and collector limits are not ceilings on separately host-authorized deeper research; provider quotas and authorized costs still apply. Scan logs in bounded inclusive block chunks with contract address and event-topic filters. Reduce chunk size on provider limits/timeouts, check pagination/result caps, retry within the applicable budget and record covered ranges. Adjacent chunks must have no gaps; remove duplicates. Exhausted budgets, rejected ranges and incomplete history mean **partial coverage**, not zero activity. Discover suitable supplemental public interfaces and sources by evidence rather than guessing API routes. Catalogs and the getter/event lists below are nonexhaustive: metadata, balances, ownership, historical events and independently supported local calculations may supplement them after provenance and semantics are established. Explorer navigation links alone do not replace required state observations.
...
An independently validated isolated nonbroadcast simulation may investigate collect, poke, burn or liquidity-change accounting under the guardrail. Record the pinned starting state, method semantics, hypothetical inputs and any isolated simulated overrides; no real wallet impersonation, credentials, signing, real-node mutation or broadcast is permitted. A successful hypothetical collection is not proof of actual collectible value, authorization, recipient receipts or a future outcome. Prefer evidenced getter/event arithmetic for the historical reconciliation; preserve rounding, principal/fee composition and simulated-versus-observed distinctions when comparing results.

## 4. Interpret the result without inventing a buyback budget

Token-unit fees are not USD profit. Prefer **“estimated uncollected LP fee balances at block B, with an indicative valuation”** when the fee component is established. Do not label that valuation collected cash, guaranteed collectible proceeds or an available buyback budget. If principal remains mixed into stored owed, label the combined amount separately. Collection rounding and applicable contract behavior still matter. Do not execute a token conversion transaction; valuation calculations and conditional conversion models remain permitted. A dashboard's displayed total cannot replace the underlying evidence.

**Denomination and freshness:** use the [pricing walkthrough](addresses-and-roles.md#read-only-pricing-walkthrough) for exact asset/feed identity, quote units, feed decimals and round validation. Read prices at the same identified block as the fee balances and report round IDs/update times, observation time and an explicit freshness criterion. A fresh block does not make a weekend-stale stock price current. With stale, missing, mismatched or unsuitable price evidence, report token balances and label any retained valuation historical/stale or unavailable, not a current quote.

For token amount `a` scaled with its token decimals and price `p` in USD per token, `a * p` is **USD**, not USDG. Establish a positive, suitable USDG/USD price `g` to convert: `value_USDG = value_USD / g`. Conversely, `u` units of USDG contribute `u * g` to a USD total. Do not convert an existing USDG-unit subtotal twice. Without a usable `g`, keep native amounts and supported USD valuations separate; a user-requested 1 USDG = 1 USD scenario must be explicitly labeled an assumption, never silently substituted for a missing read.

**Reconcile before reporting:** retain raw balances, token/feed scales, price observations and unrounded row values from one snapshot. Use exact rational/integer scaling or documented decimal precision, never binary floating point. Derive token subtotals, position values and the grand total from those same inputs; round only for display. Cross-check totals by both position and token, never mix snapshots or adjust a row to force agreement. If rounded rows do not sum to the rounded total, show the rounding residual. A larger unexplained discrepancy requires correction or an unreconciled label, not attribution to rounding without evidence.
...
When requested, calculate projected fee accrual, time to a threshold, future position value or potential buyback contributions as explicit models. State the horizon, formula and assumed volume, fee growth, range occupancy, liquidity share, prices, costs and allocation policy; show sensitivity where material. Short-period annualizations are constant-rate scenarios, not established annual returns. Unknown actual accrual or an unestablished current buyback budget does not prevent a conditional projection, but a model cannot establish realized earnings, collected cash or funds presently available to spend.
...
- Valuation denomination (USD or USDG), raw price inputs/scales, feed round/update times, freshness policy/result and any explicit parity assumption. Show position and token subtotals derived from common unrounded inputs, the reconciled grand total and display-rounding residual; do not publish an unexplained mismatch as a settled total. Separately state evidenced costs/allocations and whether a buyback budget is established.
- Requested forecasts or simulations: horizon/block, method, observed versus assumed inputs, isolated overrides if any, sensitivities and uncertainty; separate hypothetical outputs from actual accrued entitlement and receipts.
- Relevant transaction/address Robinhood Etherscan links using literal templates, plus uncovered ranges, provider errors, rounding differences and missing observations.
