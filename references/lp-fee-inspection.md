...
Use for LP earnings, uncollected fees, returned principal and their possible contribution to Sleeve buybacks. This is a research recipe, not a position operator, SDK or PnL engine. Public contract state/events are primary for contract-derived metrics; official sources support protocol claims. Dashboards are optional evidence where appropriate or when explicitly requested, never default inputs or privileged authorities. Directory order does not drive retrieval, ranking or fallback. Follow [Safety](safety.md): no wallets, collect/poke calls, liquidity changes, transaction preparation or state-changing simulations, including through `eth_call`.
