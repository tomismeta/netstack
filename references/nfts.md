...
Collection pages and APIs, including OpenSea, may be read for fresh research using ordinary host-permitted tools without a custom broker or per-source administrator setup. Public unauthenticated access is useful, but host-authorized non-wallet authentication and local research resources are also permitted subject to confidentiality and access controls. Avoid wallet extensions/providers and signing/broadcast paths. The agent must not trigger wallet prompts, connect a wallet, mint, buy, sell, list, bid, accept an offer, approve a marketplace, sign a listing/permit, submit a transaction or prepare ready-to-sign/submit artifacts, including through agent-owned wallets or delegated services. Explain the user's own evidenced purchase, listing, approval or transfer steps concretely when asked. A floor-price request is an evidence question, not permission to trade. Apply the [guardrail](../netstack-guardrail.md) and [Safety](safety.md), use minimum necessary inputs, and report unavailable evidence rather than obtaining wallet credentials or bypassing host permissions.
