...
Catalog additions observed **2026-09-26**; package version **0.4.0**. Earlier records retain their own dates. Robinhood Chain mainnet **4663**. Exact identities live in bounded JSON files routed by [address-index.json](../assets/address-index.json), not parallel copies in these references. Source chronology and review status live in [sources.json](../assets/sources.json).

## Inventory and its boundaries

The canonical inventory contains **195 distinct contract-address records: 158 named contract records plus 37 underlying price-feed contracts**. Feed proxies are contracts and are already included in 195, not an additional bucket. There are also **11 separately attributed public-role records** (ten published roles and one explorer-discovered deployer), **six trusted product mark mappings**, **seven Morpho market IDs**, and **three V4 pool IDs**. Morpho IDs, V4 pool IDs, Advance lock IDs and NFT IDs do not increase the contract-address count. The 37 feeds comprise **35 Robinhood-labelled RWA candidates** (33 explicitly `Equity`, two classification-incomplete), plus ETH/USD and USDG/USD. These are not 35 verified token mappings or 35 NetNet-used markets. Public-role and contract counts do not certify current control or activation.

The separate [deployment discovery inventory](../assets/addresses/discovery/index.json) preserves **all 53** successful indexed creations absent from the prior 185-address baseline: **four now point to canonical Asset Bond/TWAP records, ten are historically named candidates, and 39 remain unclassified**. The latter **49 are outside canonical counts**, not silently designated active products. Each row retains exact creation transaction, block and date. Three explorer pages supplied 142 rows and 138 unique successful creations; this is account-associated indexed coverage, not every deployer, ordinary transaction or recursive factory child. The discovered EOA is not attributed to a person.

Sources combine official docs, public app registries, exact historical local-registry provenance and published integration deployments. **No blanket independent deployment or safety audit is claimed.** Separate fixed-block records establish the Loopback market identity and a V4 PositionManager with two then-Sleeve-owned NFTs. Indexed creation/token evidence establishes Predict #1/#2, Asset Bond generations, oracle/Zap and Book identities within its stated limits; publisher `HUMAN-VERIFIED` remains only an attributed label. NET Advance adds explorer creation/input corroboration, CREATE-address derivation and bounded runtime analysis, not independently pinned RPC code/state or verified Solidity. Dedicated Predict, THE BOOK and Advance pages appear in the **28-entry** docs index; this is not a fresh semantic audit of all 28 pages.

## Data layout

- For a packaged lookup, start at [address-index.json](../assets/address-index.json). Its literal paths are relative to the package root: `feeds` selects a symbol file; `contracts` selects a canonical-role initial's role/alias index, then its `file`; `public_roles` lists bounded role files; `markets` points to [markets.json](../assets/addresses/markets.json). Read the selected files for an efficient exact lookup. A missing file or record is a package gap, not a research prohibition: establish uncatalogued identities with suitable host-authorized evidence under the [guardrail](guardrail.md), without guessing filenames or attributing another record's facts to the target.
...
- `publisher_linked_provenance_urls`: original publisher links retained for evidence and direct citation. Robinhood Etherscan templates are default object-navigation links, not an exclusive research or citation provider. Preserve original URLs and attribution; do not transplant provider-specific API paths or query-tab syntax to another host.
...
- `public_role_addresses`: published operational/signing/owner roles or explicitly explorer-discovered deployment roles. Preserve account-kind attribution and no inferred legal-person identity; a role named `signer` grants no signing access.
- `discovery`: a compact index and bounded candidate files, not a second production registry. Canonical-role candidates point back to their record; historical adapter/router names do not imply a current consumer. Unknown means role/activity unestablished, not active or disproven.
- `v4_pools`: exact bytes32 pool identities and separately typed position NFT IDs. Follow `pool_manager_record` and `position_manager_record` for the distinct contract addresses. Two wsNET/hOHM fee tiers are different pools, not generations of the hOHM/USDG oracle quote pool.
...
- **Loopback identity:** select `wsNET / USDG` in `markets`, also aliased `Loopback` and `Lombard Credit Facility`. Its full bytes32 ID, loan/collateral/oracle/IRM/LLTV tuple and block/hash/time are retained together. At block **67,956,523** on September 20, the public TurboRouter `marketId()` result matched Morpho singleton `idToMarketParams(id)` and the tuple's Keccak-256 identity hash. This is a dated identity/parameter observation, not a current balance, rate, liquidity, oracle-health or borrowing/liquidation-availability check. The six stock IDs and the canonical singleton route are unchanged.
...
| Asset Bond / hOHM | Robinhood hOHM token, V4 TWAP, Asset Bond Desk V2 and historical V1, conversion Zap; distinct from stock RwaDesk generations and Core bonds |
...
| NET Advance | AdvanceDesk locks/advances/repayment/close and separate AdvanceZap conversions; Sleeve is the house, not Core Treasury |
...
| HIGHER #2 / HI-2; LOWER #2 / LO-2 | `p` → [Predict series #2 outcomes](../assets/addresses/contracts/predict-series-2-outcomes.json); never aliases of #1 |
| hOHM / Origami hOHM; hOHM V4 TWAP | `h` → [Robinhood hOHM and oracle](../assets/addresses/contracts/hohm-token-and-twap.json); not the Ethereum cashtag address |
| Asset Bond Desk / hOHM Bond Desk; Asset Bond Zap | `a` → [Asset Bond V2, historical V1 and Zap](../assets/addresses/contracts/asset-bond-desk-and-zap.json) |
| NET Advance / AdvanceDesk; AdvanceZap | `a` → [Advance Desk and Zap](../assets/addresses/contracts/advance-desk-and-zap.json); not Asset Bond Desk, LPZap or Loopback |
| V4 PositionManager; wsNET/hOHM pools/NFTs | `u` → [V4 PositionManager](../assets/addresses/contracts/uniswap-v4-position-manager.json); [separate pool/position identities](../assets/addresses/v4-pools.json) |
...
Bare **HIGHER/LOWER require a series**, not automatic substitution of #1 or #2. The #2 pair has successful indexed CREATE traces from PredictDesk on September 21; the docs' first-series result does not make #1 an alias or prove #2 settlement. Generic **Pendle LP/PT/YT require a maturity**: the `p` and `y` indexes retain September 17 and October 1 identities. Resolve the requested maturity, preserving dated API lifecycle evidence rather than silently choosing one.
...
**hOHM and Asset Bonds:** resolve chain **4663** before using the token address; a social cashtag's Ethereum token is a different identity. V2's publisher-reported predecessor never listed an asset or held inventory; retain that attribution rather than claiming an independently proved empty history. An indexed deployment or a dated launch parameter is not current inventory, activation or oracle readiness. hOHM holdings, bridge/redemption claims, Asset Desk liabilities and remittance belong to separate accounting questions, not an assumed Core reserve amendment.

**NET Advance:** official docs/app role addresses agree with explorer-indexed creations at **September 25, 18:50:04 UTC, block 72,454,633**. Matching creation inputs and independent RLP CREATE-address derivation resolve the diary's swapped labels: the catalogued deployer's nonce **320** created Desk and **321** created Zap; exact transactions and runtime hashes are in the [contract record](../assets/addresses/contracts/advance-desk-and-zap.json). This establishes explorer-level provenance, not the founder's legal identity, independently pinned chain state or absence of successors. Explorer-supplied runtimes support [close/repay/preview and Zap routing mechanics](products.md#bytecode-backed-mechanics-and-their-qualifications), including a real router swap call—not an inference from approvals. Lock IDs/counts and actual executed routes still require fresh position/event evidence.

**V4 ownership is not a UI gate:** the rotated September 25 app leaves its PositionManager candidate and one NFT unpromoted. Independent RPC evidence at block **72,320,994** instead finds **two** Sleeve NFTs, **3080718** and **3269350**, in distinct **0.05%** and **0.9%** wsNET/hOHM pools. Complete filtered incoming transfers plus owner rechecks matched `balanceOf(Sleeve)=2` at that block. These are dated identities, never seeded substitutes for fresh full ownership discovery. The separate hOHM/USDG oracle-pool ID is corroborated by indexed bytecode; its complete PoolKey is not established in this catalog. No pool ID is a 20-byte address.

For website labels and the corresponding RPC/manual route, see [Website metric meanings](website-metrics.md). Static site code supplies identity/interface/methodology provenance, **never numerical inputs** to the collectors; a later asset rotation does not refresh an earlier observation.
...
**Six mappings; 29 unmapped:** join “Existing exact mapping” to `trusted_product_marks` by chain and full `feed_address`, then resolve `token_address`. These are historical local-registry relationships, not independently verified current pairings. The other **29 have no exact token relationship or NetNet-use evidence recorded**; bounded public directory/documentation lookup established no additional pairs. Ticker matching cannot fill the gap, but new evidence may establish a relationship for research without prior package inclusion.

**Address types are not interchangeable:** directory `contractAddress` is an aggregator; neither it nor `secondaryProxyAddress` identifies a token. `trusted_product_marks.source_contract_address` is historical provenance—not a Chainlink proxy, recommended spender or current-wiring proof. NVDA cites superseded Desk V1; AAPL cites V2. Read feed answers from the evidenced feed proxy, not those desks. Research may establish additional pairings; package curation should update canonical records rather than create a competing pairing inventory.
...
For an **observed Credit collateral or Board Meeting entry mark**, identify the exact oracle/adapter, interface, output scale and raw/display unit basis, and read **the product's own result** at the observation block. A derived reconstruction is also possible when the applicable arithmetic and inputs are established; the direct-feed formula alone does not establish the deployed product formula. If actual arithmetic or inputs remain unknown, investigate or show a clearly labeled hypothetical model with explicit assumptions, not an exact deployed output. [Products](products.md#oracle-and-liquidation-risks) and [Games](games.md#the-board-meeting--counterparties-choose-reward-systems-differ) retain the product-specific claims.
...
Current NET observations require the chain/head-recency checks below plus the applicable interface, token ordering, units/decimals, window and source state; packaged metadata supplies no live facts. **USDG is not USD:** an observed USD conversion needs a separately evidenced USDG/USD mark, scale and freshness. A hypothetical peg or other conversion assumption may be modeled if labeled. Report observed, derived and modeled spot, TWAP, NAV and collateral values separately from a policy bid. Do not pass assumptions off as unavailable observations. The agent must not checkpoint, trade, borrow, approve or connect a wallet, but may explain user-operated steps and perform established non-broadcasting research simulations.

### Read-only pricing walkthrough

For “display a USD reference price for this NVDA token”:

1. **Match identity.** Establish the requested chain ID and full token address. If they match NVDA's `trusted_product_marks`, resolve `feed_address` to `chainlinkNvdaUsd` and report local-registry provenance. For a different or uncatalogued token, stop reuse of that mapping—not research—and establish its exact feed relationship from suitable evidence; ticker alone is insufficient.
...
3. **Establish observation recency.** Under [Safety](safety.md) and [public RPC guidance](integrations.md#read-only-verification-workflow), confirm chain **4663** and a recent canonical head. Record block number/hash/time and retrieval time; check head lag against an explicit use-case bound. `latest` alone proves nothing. Read `decimals()`, `description()` and `latestRoundData()` from the exact proxy **at that same block** for this direct-read walkthrough. An old block's then-fresh round is historical, not current. Unestablished head recency requires a historical/recency-unverified label. Unavailable access never justifies invented observations, real wallet impersonation or real-node mutation. Isolated, verified non-broadcasting simulations, including simulated overrides, may support labeled analysis under the [guardrail](guardrail.md), not claims of observed state.
...
6. **Keep the mark's limits.** A feed mark is not an executable quote, liquidity, collateral backing or redemption right. `rialtoRouterRegistry` and [Rialto docs](https://docs.rialto.xyz/developers/router-registries.md) identify conversion infrastructure, not current quotes or permission for the agent to trade. Research read-only quotes when their non-broadcasting behavior and applicability are established. Explain [game conversion steps and limits](games.md) concretely for the user; do not connect a wallet or prepare ready-to-sign/submit artifacts.

**Answer scope:** a default directory response includes all 35 RWA roles, ETH/USD and USDG/USD, four NET source kinds, quote/scale, mapping/classification gaps and provenance boundaries. A focused response includes the selected relationship, consumer limits and missing evidence, with supplemental discovery when needed. Literal bounded JSON files are lookup aids; neither “not recorded” nor packaged metadata establishes verification or a current price.
...
- **Predict:** Desk and House Vault are separate from series-specific HIGHER/LOWER tokens. Dedicated docs now specify Safe grading, tie/void and queue rules and report a first result; publication does not verify deployed enforcement or payout transfers. Reports total, Predict display and authoritative Safe-posted print have different evidence and accounting requirements.
...
- **Asset Bond generations:** `assetBondDesk V2` is not stock `RwaDesk V2`. V1 remains historical; TWAP and Zap are separate contracts. A community discount claim, publisher initial configuration or generic treasury announcement cannot substitute for same-block state or establish Core RFV inclusion.
- **V4:** singleton PoolManager, periphery PositionManager, pool ID and NFT ID are four different object types. Publisher promotion is not evidence of zero/nonzero ownership; independently discovered positions must be included or explicitly marked missing, never silently omitted because the site hides them.
...
- **NET Advance:** fixed Desk and Zap roles do not fix a forever-current generation or lock universe. Distinguish user-locked wsNET, house inventory, unallocated USDG and Treasury-share escrow. Documented maturity/keeper behavior is not automatic settlement; follow [mechanics and evidence limits](products.md#net-advance-selling-future-rebases-not-guaranteed-principal).

## Read-only identity procedure

1. For a packaged identity, follow the root index to the exact role/source's bounded file. For an uncatalogued identity, use suitable source evidence. In both cases establish **chain ID plus full address**; aliases help discovery but are not identifiers.
...
7. Link the exact public record using validated chain and object identity, including for historical deployments or transactions. Robinhood Etherscan is the default object link, not a restriction on alternative providers or direct evidence citations. Explain what was checked and what was not, preserving source/block dates and original attribution. Never verify by connecting, approving, signing, deploying or transacting.
