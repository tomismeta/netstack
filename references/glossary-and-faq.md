...
The [dashboard directory](docs-and-sources.md) lists [NetNet Navigator](https://netnet.exe.xyz/) first for presentation only, not as a preferred analytic source. It is an independent dashboard, not official protocol authority or a package dependency. Compare observations by relevance, definitions, provenance, block/time, scope and completeness; no list position supersedes published terms or a verified contract observation. No transaction, wallet, signature or execution instruction is needed to explain these distinctions.
