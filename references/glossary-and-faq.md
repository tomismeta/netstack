...
| RWA Sleeve | Manager-custodied assets and off-wallet claims outside Core. Reports adds stocks/cash, Credit/LP/TURBO, Predict, Book, configured Asset Desk assets and own NET-family holdings; gross pots and omitted fees/liabilities keep the memo distinct from net equity. | [Reports scope](rwa-strategy.md#official-reports-sleeve-memo-methodology) |
...
| Predict / outcome token | Weekly binary exposure with one HIGHER/LOWER pair per series. Its documented settlement mark excludes RWA NET inventory, freezes opening House value and includes certain fees omitted by Reports; neither is Core RFV. | [Predict docs](https://docs.netnet.capital/predict), [scope](products.md#netnet-predict-weekly-outcomes-and-house-vault) |
| House Vault | Nontransferable internal shares in loss-bearing Predict underwriting; pending deposits, active shares and matured claims differ. Not lending, AMM liquidity or guaranteed cash. | [House terms](products.md#house-vault-underwriting-not-passive-savings) |
| Asset Bond Desk / hOHM | Separate asset-bond family and announced Olympus/Origami partnership, not old RWA remittance rules or evidence hOHM entered Core reserves. | [Evidence and boundaries](products.md#asset-bond-desk-and-the-hohm-partnership) |
| Reports “True RFV” / adjusted net assets | Core plus the publisher's Sleeve memo / a separate net-asset analysis accounting for supported liabilities, fees and exclusions. Unknown components keep totals partial or unavailable, not zero. | [Four scopes](rwa-strategy.md#four-values-four-questions) |
...
### Which route answers “Treasury,” “True RFV” or “net assets” on the website?

Use `analytics.py rfv --scope core`, `--scope reports` or `--scope net-assets`, respectively, and retain the output's coverage gaps. The website supplies labels/methodology, **never live numerical inputs**; all dependent values must be pinned RPC reads. “NAV incl. RWAs” is memo/total supply, while “True NAV (circulating)” uses another denominator. Predict's Safe-posted print has its own settlement specification and cannot be replaced with either current total.
...
### Can you explain how to bet or model future returns?

Yes. The [guardrail](guardrail.md) constrains agent execution, not the user's questions. Explain evidenced user-operated mechanics, including navigation, selection, stake/assets, fees, approvals, confirmation, settlement and withdrawal. For a Giants question, identify the intended game and matchup rather than assuming a fantasy game or inventing a market; [Games gives a concrete conditional THE BOOK walkthrough](games.md#explaining-how-the-user-bets-on-the-giants). The agent does not connect wallets, access wallet credentials, sign, approve, place bets or prepare ready-to-sign/submit artifacts.

Calculations of accrual, future prices, expected returns, probabilities and time to a threshold are allowed when a model can be stated. Label observed inputs, derived results and modeled assumptions separately; investigate missing inputs or show bounds/scenarios instead of inventing an exact result. Unknown deployed arithmetic does not prohibit a hypothetical model, and a short-period annualization may illustrate a constant-rate scenario without promising that rate will persist. Keep the requested scope: do not add forecasts to an observations-only reconciliation.
