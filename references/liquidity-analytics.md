...
Use for **current LP holdings**, **period liquidity flows** and **swap-fee generation/attribution**: three different measurements. This concerns the canonical fungible v2 LP receipt, not the Sleeve's [v3 NFT fee-growth recipe](lp-fee-inspection.md) or Predict's House Vault. V2 has no per-holder `feeGrowthInside` or separate collectable-fee balance. The [research guardrail](../netstack-guardrail.md) and [Safety](safety.md) govern retrieval and execution: concrete user-operated explanations are allowed, but the agent must not operate wallets, submit transactions or prepare execution-ready transaction artifacts. Separately validated read-only quotes and isolated nonbroadcast simulations are allowed; they must not involve real wallet credentials, signing, live-chain mutation or submission. Dashboards are optional evidence.
...
- Define chain **4663**, exact question, holder versus beneficial-owner scope and interval `(A,B]`. Default to the requested seven days, locating `A` by block timestamps and reporting the actual boundary difference. If the request allows a shorter interval, label its exact duration. Otherwise an incomplete seven-day scan remains an incomplete seven-day answer: never silently substitute a shorter complete period or present its extrapolation as observed seven-day activity. A requested extrapolation may be shown separately as a model with explicit assumptions.
...
- Follow the shared [quick-pass defaults and collector limits](integrations.md#live-analytics-execution-limits) before retrieval; requested deeper research may use a suitable host-authorized scope beyond those defaults. First preserve required pair/factory identity, token units, supply, reserves and balances at B. These can support a pool snapshot, **not complete holder discovery or third-party ownership**. Then prioritize creation-to-B LP Transfers and `(A,B]` fee evidence required by the question; do not expand a holdings/fees request into unrelated flow or cost-basis research. If an uncached value at B becomes unavailable through pruning, retain the missing value or restart the entire dependent snapshot within the applicable research budget; never splice latest state into the old snapshot. Logs may remain available when archive `eth_call` state does not.
- Verify current interface and implementation applicability. The [bundled read/event ABI](../assets/analytics/v2-interface.json) comes from pinned upstream v2 source; the app separately confirms a subset. Neither is an exhaustive research allowlist: supplemental interfaces, metadata, balances, ownership, historical events and local calculations may be investigated with established provenance and semantics. Successful getters or matching event topics alone do not prove fee constants, token behavior or deployed-code equivalence. Use verified deployed source/bytecode evidence where available; otherwise label standard-v2 calculations **conditional**, not audited facts.
...
- For the default quick pass, stop at the first shared deadline or recipe cap: **250 RPC members including retries and 50,000 log rows total**. These defaults and the runner's enforced collection limits do not cap separately host-authorized deeper research. Record the applicable budget and usage. Start with address/topic-filtered chunks no larger than 1,000,000 blocks; adjust chunking and recovery to provider constraints and the authorized research scope. `eth_getLogs` has no standard cursor: partition inclusive block ranges, or exhaust documented pagination. Adjacent ranges must have no gaps. Suspected silent truncation, capped pages or a missing single-block page prevent completeness; retain the gap if further splitting cannot finish in budget. Deduplicate by block hash/transaction hash/log index and sort by block, transaction index, log index. Exhaustion or unavailable archive state is **missing evidence**, not zero activity.
...
Actual standard burn arithmetic uses token **balances**, floors each token output and first may mint protocol-fee LP, diluting existing receipts. NET's separately documented transfer levy may further reduce recipient proceeds. Present pending dilution and `Q-R` separately; do not label reserve-share amounts exact net withdrawable cash. Evidenced local arithmetic or an independently validated isolated nonbroadcast burn/sync/skim simulation may investigate these effects under the guardrail, never execute them on the live chain. State the starting block, simulated inputs and any isolated state overrides; do not impersonate a real wallet or mutate a real node. Hypothetical outputs are not actual collectible proceeds or cash received. Keep integer/rational calculations unrounded until display; a raw fractional claim is an accounting proportion, not a transferable fractional base unit.
...
For requested forecasts, model projected fees, LP value, dilution or returns separately from this historical reconciliation. Identify evidenced inputs versus assumed volume, fee policy, ownership, reserves, token levies, prices and costs; state the horizon and sensitivity to changes. Short-window fee annualizations are constant-rate scenarios, not observed annual earnings or promised APR. Missing historical ownership or actual accrual does not prevent a conditional model, but its output cannot fill an observed-evidence gap.
...
| Requested models | Projected fees/value/return, horizon, formula, observed versus assumed inputs and sensitivities; separate from realized or historically attributed results |
