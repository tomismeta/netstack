...
- Collect/cache required same-block state promptly, before long history scans. If an uncached value at B becomes unavailable through pruning, retain the missing value or restart the entire dependent snapshot at a new B; never splice latest state into the old snapshot. Logs may remain available when archive `eth_call` state does not.
...
- Set a bounded budget before retrieval, for example **250 RPC requests, 50,000 log rows and 10 minutes**, including receipts/retries. Record the budget actually used. Start with address/topic-filtered chunks no larger than 1,000,000 blocks; reduce on errors, timeouts or caps. `eth_getLogs` has no standard cursor: partition inclusive block ranges, or exhaust a provider's documented pagination. Adjacent ranges must have no gaps. Suspected silent truncation, capped pages or a missing single-block page prevent a completeness claim; split further or report the gap. Deduplicate by block hash/transaction hash/log index and sort by block, transaction index, log index. Exhaustion or an unavailable archive read is **missing evidence**, not zero activity.
