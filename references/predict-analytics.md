...
1. Pin/recheck chain 4663, Desk identity/code, `seriesCount`, selected tuple, timestamps, halt, `minTicket`, USDG and outcome decimals at B. The quote signatures have **no series ID**: establish which current series they price at the quote's block; do not apply them to a historical row or reuse an earlier snapshot's routing assumption. Reading only one series does not establish that it is the only open series.
...
**Residual triage:** retain `Desk balance - (selected series.bookUsdg + selected series.vaultFees)` as a signed diagnostic, not an unexplained asset or liability to allocate automatically.

- Confirm all inputs share the same block, token and raw units; distinguish whole-Desk custody from one series' accounting.
- Inspect `reservedUsdg`, `pendingNetUsdg`, `prizeCarry` and relevant series' prize/settlement state. Treat these as potentially overlapping observations, not an additive reconciliation formula.
- Trace actual USDG transfer/receipt legs for funding, deferred NET buys, fee routing, prizes and remaining claims. Record covered and missing history and associate each proposed explanation with evidence.
- House queued deposits and matured withdrawal notices belong to a different custody/accounting scope. Do not use them to explain Desk cash without establishing the corresponding transfer or obligation.
- Preserve any remaining residual as unresolved. A snapshot-only request need not trigger a history sweep; disclose the next bounded investigation instead.
