...
- Collect/cache the required same-block state before a long history scan. Pruning of a missing read at B is not permission to use latest state beside old balances: retain the gap or restart all dependent snapshot reads at a new B. Historical logs can remain available when archive state does not.
...
- Set explicit budgets before retrieval: for example at most 100 RPC requests, 20 series per discovery page, 100,000 blocks per initial log chunk, and 10,000 decoded logs per page. These are research limits, not claimed provider limits. Count retries and dependent token/account reads. Shrink rejected chunks; follow documented provider pagination and detect truncation/caps. Continue only within the budget or return the exact uncovered ranges/IDs. Do not call a result complete because a page was empty.
...
If event ABI, historical ranges, receipts or archive reads are unavailable, return the useful pinned snapshot: series identities/states, named getter counters, balances and House fields. Label flow metrics and decomposition **unavailable/partial**, not zero. Getter totals are not a substitute for absent buy/sell/redemption history. An exact observed partial sum can be reported with its range, not as “since opening.”
