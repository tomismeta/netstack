...
RFV optionally accepts `--detail summary`; **`--detail full` is the unchanged default**, and LP/Predict/House/Advance do not accept this option. Summary changes stdout presentation only, retaining snapshot/recheck, provenance, status/errors, coverage/missing evidence, methodology limits and all accounting views with balanced component inclusion/economic treatment. It explicitly identifies omitted raw/repeated evidence and same-document references. Add `--output PATH` to that same invocation when full evidence is wanted: checkpoints remain **full**, never the summary. Without `--output`, omitted evidence is not saved; no automatic file or repeat collection is created. A new run is not the same observation. See [detail and checkpoint semantics](reserve-analytics.md#stdout-detail-and-same-collection-evidence).
...
- **Advance:** pinned desk/zap/token bindings, parameters and balances precede position discovery. Enumeration is complete only with a supported count reconciliation; failed ranges or required getters leave full-universe aggregates null and preserve known rows/subtotals. `advanceOf(id)` is actual advance, not `owed/1.15`; opening needs advance plus Treasury reserve, not just the reported cash capacity. `unlockAt(id)` is a timestamp, not automatic closure. Under the [reviewed runtime](products.md#bytecode-backed-mechanics-and-their-qualifications), `paid` mixes USDG repayment and collateral credit, and preview uses a different index path from actual close. `interface.runtime_evidence` retains that explorer provenance; the collector's code-presence check does not hash-match it. Open/closed/repaid classifications and fee entitlement do not establish profit or remittance. Retain source/version and unofficial-analysis qualifications; Reports reserves are a separate composition, not collateral plus receivables plus cash.
