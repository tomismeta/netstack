...
Use ordinary host-authorized research under [Safety](safety.md#everyday-public-research) and the [governing guardrail](../netstack-guardrail.md); no custom broker or per-public-destination setup is required. Public unauthenticated access is a starting point, not a global limit: approved non-wallet authentication and explicitly authorized local research resources are allowed with confidentiality and SSRF protections. Explain concrete user-operated actions, including betting, without operating wallets. Agent-authored code, ordinary research delegation, new sources/interfaces, models, forecasts and accrual calculations are permitted with provenance and explicit observed/derived/modeled distinctions. Established read-only quotes and isolated non-broadcasting simulations may be used without real-wallet credentials, signing, live-chain/node mutation or submission.

## Live analytics execution limits

For the supported **NET/USDG v2, Predict and House collector scopes**, prefer the optional reviewed [runner](../scripts/analytics.py) under normal host permission. It uses existing **Python 3.10+ on Linux/macOS, standard library only**, a fixed public Robinhood RPC origin and packaged canonical routes/ABIs. There is no custom-endpoint, credential, wallet or arbitrary-RPC mode. Read the matching accounting recipe before interpreting its output. Knowledge-only use remains available without Python.

Those three runner commands are an implementation scope, **not a boundary on research**. [THE BOOK's snapshot-first workflow](games.md#book-snapshot-first) uses a [small read/event interface](../assets/analytics/book-interface.json) with ordinary host-authorized reads; it has no runner command. The whole-answer allowance and manual-research budgets below are **quick-pass defaults**; runner caps remain fixed to that helper. Requested deeper research can set a deliberate finite scope and allowance within actual host deadlines, provider quotas and approved costs. Discover additional interfaces or use agent-authored code where needed, with established semantics, common-block snapshots and honest coverage. No wallet action, untrusted source-code execution or denied-command bypass is authorized.
