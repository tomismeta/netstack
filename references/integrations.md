...
RFV and Advance accept `--detail summary`; **full JSON remains the default**. `--output PATH` always saves full same-collection evidence; without it, omitted detail is not saved. Advance capacity/params summaries show native amounts, halt status, snapshot, verification, errors and coverage gaps; provenance summaries show offline identities, evidence classes and URLs. They omit routine metadata and narratives, retaining partial-only quantities when necessary. Full and summary share the same `verification` and amount objects (`raw`, `token`, `decimals`, `formatted`, unit-bearing `display`), computed before checkpoint serialization. Missing decimals leave formatting unknown; no floats or assumed scales. Immediate failures still carry explicit verification limits. Detailed reads remain in the full checkpoint; a new run is not the same observation. [RFV detail semantics](reserve-analytics.md#stdout-detail-and-same-collection-evidence) remain unchanged.
