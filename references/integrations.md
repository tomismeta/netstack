...
For NET Advance, use `python3 -I -B scripts/analytics.py advance --view capacity --deadline 120 --json` for a pinned state snapshot, or `--view params` for parameters; neither scans position history. `--view positions`, `holders` or `totals` (default) discovers actual opening IDs through events and reconciles them against the same-block count. These views share one position collection; no saved IDs or “two empty slots” heuristic completes the universe. Use an explicitly permitted longer deadline for history, not an automatic backfill.

For catalog-only inspection, `python3 -I -B scripts/analytics.py advance provenance --json` resolves Desk/Zap identities, deployment provenance and cited source records **without network calls**. It does not verify live state or the host-loaded revision. General bounded address lookups use the indexed catalog rather than a `lookup` CLI command.
...
RFV and Advance accept `--detail summary`; **full JSON remains the default**. `--output PATH` saves full same-collection evidence; without it, omitted detail is not saved. Advance capacity/params summaries show native amounts, `desk_halted`, snapshot, verification, errors and coverage gaps; provenance summaries show offline identities, evidence classes and URLs. Representative successful capacity/provenance summaries have a **4 KiB regression ceiling**, not a runtime truncation limit: failures/partial evidence may exceed it. Full and summary share `verification`, `coverage` and amount objects (`raw`, `token`, `decimals`, `formatted`, unit-bearing `display`). Coverage gap arrays always live at `coverage.required_missing` and `coverage.supplemental_missing`, including before any successful read. Missing decimals leave formatting unknown. Full evidence preserves the Desk getter's raw `halted` name; **Desk false does not establish Zap halt status**. Detailed reads remain in the checkpoint; a new run is not the same observation. [RFV detail semantics](reserve-analytics.md#stdout-detail-and-same-collection-evidence) remain unchanged.
...
- `--block` accepts an explicit block number or `latest-2` (default). The same B governs every dependent getter, including all House account reads. Two-block lag is not a finality guarantee. `--output PATH` optionally preserves a full checkpoint **outside the installed package**; it is research evidence, not a reusable current-holder list or authority to modify the package. Do not send private paths or checkpoint contents to an RPC endpoint.
- Within the **bundled collector**, each attempt is bounded by the remaining budget and at most **15 seconds total wall time**; socket read timeout alone does not stop trickle responses. **RFV/Advance have 750 RPC members; LP/Predict/House retain 250**, including batches and retries. LP has at most **50,000 log rows**, Predict/House/Advance **10,000**. Batches have at most **20 members**, matched by response ID, not position. Preserve successful members; retry transient failures at most once and use at most **10 recovery attempts** overall, including splits/fallback. These helper caps are not global research limits. Never bypass denials; malformed ABI or pruned state needs diagnosed interface/archive recovery, not blind retries.
...
- **Advance:** pinned bindings precede position discovery; failed ranges or required getters withhold full-universe totals while preserving observed rows. `advanceOf(id)` is actual advance, not `owed/1.15`; opening needs advance plus Treasury reserve. `unlockAt(id)` is not automatic closure. Under the [reviewed runtime](products.md#bytecode-backed-mechanics-and-their-qualifications), `paid` mixes cash and collateral credit; preview and actual close use different index paths. `verification` separates observed live getters from `analyzed_runtime_match: not_checked`. Full JSON keeps analysis at `provenance.analysis` and shared per-read block/provider context at `metrics.read_context`; references are same-document JSON Pointers. Zap halt status is a known interface gap, not evidence of an additional hidden halt mechanism. Observe Desk `halted` for Desk opening restrictions; dependency liveness and end-to-end execution remain separate gaps. Entitlement is not remittance or profit; borrower collateral is not protocol backing.

RFV inventories attach searched owners/contracts, candidate provenance, raw quantities and an explicit disposition: included, excluded with reason, unpriced, ownership unresolved or uninspected. Supplemental ERC-20 incoming-Transfer discovery searches the latest **1,000,000 blocks**, newest first in at most **100,000-block requests**; earlier history and other token standards/indirect claims remain outside that window. V4 instead discovers incoming transfers afresh in the pinned manager from genesis toward B, processing newest ranges first under the same finite budgets; no dated NFT seeds are executable inputs. Exact current `ownerOf`/`balanceOf` cardinality can prove that manager's direct inventory without complete historical activity. An owner revert is not proof of a burn: it remains unresolved unless independent current-count reconciliation excludes the historical candidate. Unreconciled inventory withholds both owned-universe totals and publisher principal impact; reconciled publisher-selected principal remains separate from unselected quantities and pending fees. Supplemental holdings outside Core's formula are not added to Core RFV.
