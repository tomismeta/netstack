...
| Stock-token fees are valued at 200 USD, with a separate 50 USDG fee balance and a same-block USDG/USD price of 0.80. What is the total? | Synthetic: 300 USDG or 240 USD, not 250 USDG. Keep denominations explicit; without a usable USDG/USD price, do not silently assume parity |
| A fresh block contains an old stock-price round. Is the LP fee valuation current? | Token balances can be observed at that block, but freshness of each price is separate. Apply an explicit criterion; label stale/missing valuation evidence, not a current price |
| Token subtotals are 1,528.28 and 1,642.75 USDG, but the reported grand total is 3,170.95. Is it reconciled? | No: displayed subtotals sum to 3,171.03. Inspect common unrounded inputs and denomination; correct or flag the 0.08 discrepancy instead of dismissing it as rounding |
| An earlier report listed four LP NFTs. Rebalancing replaces some IDs and adds a position in a pool absent from the catalog. Should you reuse that list? | No. Rediscover all owned IDs at the new block, verify owners, re-read surviving positions and resolve every discovered pool through the factory. Historical IDs/counts and known-pool hints must not restrict discovery |
