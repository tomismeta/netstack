...
For smaller RFV or Advance stdout, add `--detail summary`. To retain full evidence from that **same collection**, also pass `--output /absolute/path/outside-the-skill/evidence.json`; the file stays full regardless of stdout detail. Summary output preserves accounting and missing coverage, identifies omitted detail and never starts a second collection. Without `--output`, omitted evidence is not saved. Advance summaries include exact decimal-formatted amounts when token decimals were observed, alongside raw integers; missing decimals remain unknown.
...
For an **offline** Desk/Zap identity and evidence lookup, run `python3 -I -B scripts/analytics.py advance provenance --json`. This reads the packaged catalog, not the network. General bounded address lookup still means navigating [the address index](assets/address-index.json), not an unimplemented `lookup` command. Advance output distinguishes observed live reads from `analyzed_runtime_match: not_checked`; code presence and pointer checks do not match the analyzed runtime hash. Zap halt status is unknown because no separate getter is exposed, not `false`.
